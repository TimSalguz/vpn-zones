...
- **`harness.nix`** — автономная обвязка без flake-инпутов (у `flake.nix` их
  ...
  - `scripts.<имя>` — каждая script-деривация модуля адресно
    ...
    Rust-крейт сюда не входит — у него свой job. `vpn-zone` теперь
    двухстрочная обёртка над бинарём крейта: она назначает `VPN_ZONE_TOOLS`
    (манифест путей инструментов) и делает exec;
...
3. **rust** — крейт `rust/` (жизненный цикл зоны, парсер конфигов WG/AWG,
   ...
   системы `fs-sandbox` и сам CLI `vpn-zone`) в
   ...
   `systemctl` или `nsenter` — то, что их требует, проверяет смоук. В деривации
   модуля тесты выключены (`doCheck = false`): там их гонять негде.
