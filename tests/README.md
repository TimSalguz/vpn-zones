...
3. **rust** — крейт `rust/` (парсер конфигов WG/AWG, генератор
   seccomp-фильтра, слои профилей, генератор `.desktop`-ярлыков и
   ...
   покрывают грабли §4, §5, §7 и §10 из `docs/GOTCHAS.md` (разбор `.desktop`,
   отсутствие кавычек в `Exec`, битые симлинки, идемпотентная запись,
   уборка чужого не трогает). У `wl-sandbox` проверяются fallback-пути:
