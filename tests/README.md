...
- **`harness.nix`** — автономная обвязка без flake-инпутов (у `flake.nix` их
  ...
  - `ocTools` — отдельный buildEnv для части смоука про зону OpenConnect:
    `ocserv` (настоящий сервер AnyConnect), `openconnect` (тот же клиент, что
    поедет пользователю) и `openssl` (сертификат, ключ и crypt-хеш пароля).
    Отдельным атрибутом, потому что замыкание `ocserv` больше нигде не нужно.
...
- **`vm.nix`** — VM-тест на фреймворке NixOS-тестов: qemu-машина с настоящим
  ...
  Здесь же — единственное покрытие НАСТОЯЩЕГО пути AmneziaWG. Обе VM грузят
  внешний ядерный модуль `amneziawg`
  (`boot.extraModulePackages = [ config.boot.kernelPackages.amneziawg ]`),
  поэтому держатель зоны идёт своей ОБЫЧНОЙ веткой — `ip link add awg0 type
  amneziawg` и настройка утилитой `awg`, — а не fallback'ом на ядерный
  `wireguard` (fallback остаётся за смоуком в CI: на раннере модуля нет, и
  ветки поделены между двумя тестами). Проверяются обе формы:
  - зона `vmreal` — конфиг БЕЗ параметров обфускации против обычного
    wireguard-пира на сервере: `ip -d link show awg0` обязан показать тип
    `amneziawg`, а рукопожатие, трафик и DNS обязаны работать (AmneziaWG без
    junk-параметров совместим с чистым WireGuard по проводу);
  - зона `vmawg` — конфиг с НАСТОЯЩЕЙ обфускацией (`Jc`/`Jmin`/`Jmax`,
    `S1`/`S2`, `H1`–`H4`) против второго интерфейса сервера (`awg1`, отдельный
    порт 51821, отдельная подсеть 10.98.0.0/24, те же параметры обфускации,
    настроен через `awg setconf`). Обычный WireGuard такому клиенту не ответил
    бы вовсе, поэтому само рукопожатие и есть доказательство, что параметры
    доехали из конфига в ядро на обеих сторонах. Ассерты: зона поднимается,
    TCP через туннель доходит до ответчика на `10.98.0.1:8081`, после этого
    `vpn-zone check` видит рукопожатие, а tcpdump-капкан на `eth1` («всё в
    сторону сервера, кроме UDP порта туннеля») остаётся пустым. Порядок
    важен: сама по себе зона в сеть ничего не шлёт, а WireGuard (и AmneziaWG
    вместе с ним) начинает рукопожатие только когда есть что передать —
    спрашивать `check` до трафика значит ждать вечно на исправном, но
    молчащем туннеле.
...
5. **vm** — VM-тест `tests/vm.nix` (см. выше): правило udev открывает
   ...
   единственный job, где есть сессионный systemd, живой WireGuard-сервер и
...
- **зона OpenConnect** — единственное покрытие второго типа бэкенда, и оно
  сквозное. На раннере под `sudo` поднимается настоящий `ocserv` с
  сертификатом и паролем, сгенерированными на лету (в git не попадает ни
  ключа, ни отпечатка), в сети ХОСТА: свой tun и привилегии внутри
  непривилегированного userns ему не получить, а зоне до него ходить всё равно
  незачем иначе, чем через pasta — то есть ровно тем путём, которым она ходит
  к настоящему шлюзу. Отпечаток сертификата спрашивается У САМОГО КЛИЕНТА
  (`openconnect` печатает готовую строку `--servercert pin-sha256:…`), и он же
  обязан ОТКАЗАТЬСЯ соединяться без пина — отключать проверку мы не умеем
  нигде. Дальше: `vpn-zone add` обязан отвергнуть тот же конфиг с `--script` в
  `Args`; зона поднимается держателем; в app-ns снова РОВНО два линка, `awg0`
  UP, default и маршрут до самого шлюза — оба в туннель; в `resolv.conf`
  резолвер и `search`-домен ОТ ШЛЮЗА, а не от хоста; в аплинке есть pasta и
  НЕТ `awg0` (переезд tun состоялся); правило второго эшелона аплинка сужено до
  адреса шлюза; из зоны открывается TCP до дальнего конца туннеля
  (`192.168.222.1:4443`) — единственный способ отличить «настроено» от «трафик
  идёт»; зеркало состояния говорит `backend: openconnect` и `connected: yes`,
  `vpn-zone check` признаёт зону живой; и, наконец, клиента убивают — оба
  namespace обязаны исчезнуть, потому что это и есть fail-closed. Часть
  требует `sudo` без пароля: без него она пропускается с явным сообщением, а в
  CI пропуск запрещён переменной `VPN_ZONE_SMOKE_REQUIRE_OC=1`;
...
отсутствии `DNS=` в конфиге. Это сделано намеренно: fallback-ветку держит
смоук, а обычную awg-ветку (с модулем и с настоящей обфускацией) — `vm.nix`.
...
`smoke-pickapp` (плюс зону `ocsmoke` и каталог `/tmp/vpn-zones-ocserv.*`, если
есть `sudo` без пароля) — в начале удаляет их остатки, чужие зоны, профили и НАСТРОЙКИ
