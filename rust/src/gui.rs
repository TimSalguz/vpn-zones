        ...
        "mode" => {
            let Some(value) = menu_with_default(
                ...
                &[
                    ...
                    row(
                        "per-zone",
                        "Отдельный ярлык на каждую зону (устарел, будет убран)",
                    ),
                    row("both", "И то, и другое (устарел, будет убран)"),
...
fn forget(tools: &Tools) -> u8 {
    ...
    if choice == "__all__" {
        ...
        dialog::notify(
            ...
            "5000",
            "Сброшено",
            "Контейнер снова спрашивается для всех программ; сети контейнеров остаются их.",
        );
    } else {
