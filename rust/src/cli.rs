...
pub fn main() -> ExitCode {
    ...
    let code = match verb.as_bytes() {
        ...
        b"question-timeout" => term_setting(&tools, rest, &crate::timings::QUESTION, |t| {
            match t.duration() {
                Some(_) => format!(
                    "вопрос брокера ждёт ответа {}, потом закрывается отказом",
                    t.text()
                ),
                None => "вопрос брокера ждёт ответа без срока".to_owned(),
            }
        }),
        b"handshake-check" => term_setting(&tools, rest, &crate::timings::HANDSHAKE_CHECK, |t| {
            format!(
                "добавив зону, окно ждёт рукопожатия {} перед тем, как сказать, жив ли туннель",
                t.text()
            )
        }),
...
/// Whether a zone `systemctl start` has just returned for is up: the `ready`
/// marker AND a live zone process. No waiting here, and no clock: the unit is
/// `Type=notify`, so `systemctl start` returns when the holder says the zone
/// is ready (`READY=1` once `ready` is written, `zone::hold`), or when it
/// failed — however long its setup takes on a loaded machine (this was ten
/// seconds of polling, and a slow start said "не поднялась" of a zone that
...
pub fn started_up(state: &Path, name: &OsStr) -> bool {
...
/// `cellward <setting> [<term>|never|default]` for a term setting of
/// `crate::timings`: shown without a value; a value within its bounds
/// written; `default` removes the local one. What Nix declares is changed
/// in Nix.
fn term_setting(
    tools: &Tools,
    args: &[OsString],
    setting: &crate::timings::Setting,
    say: fn(crate::timings::Term) -> String,
) -> u8 {
    use crate::container::Source;
    let show = |tools: &Tools| {
        let (term, source) = setting.read(&tools.config);
        let from = match source {
            Source::Local => String::new(),
            Source::Nix => format!(" (задано в Nix: programs.cellward.{})", setting.nix),
            Source::Default => " (умолчание)".to_owned(),
        };
        println!("{}{from}", say(term));
    };
    let value = match args {
        [] => {
            show(tools);
    ...
    let written = if value == "default" {
        if tools.config.join(DECLARED_DIR).join(setting.name).exists() {
            ...
            match fs::remove_file(tools.config.join(setting.name)) {
                Err(e) if e.kind() != std::io::ErrorKind::NotFound => Err(e.to_string()),
                _ => Ok(()),
            }
        }
    } else {
        match setting.parse(value) {
            Some(term) => write_setting(tools, setting.name, OsStr::new(&term.text())),
            None => {
                eprintln!("срок — число и единица ({}), или default", setting.bounds());
                return 1;
            }
        }
    };
