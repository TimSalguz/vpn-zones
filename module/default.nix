...
let
  ...
  # --- ЧАСТЬ 0б: RUST-ЯДРО (ПОКА — ТОЛЬКО SECCOMP-ФИЛЬТР) ---
  # Отсюда начинается переезд ядра на Rust (ROADMAP M1/M2). Первым переехало
  # то, чего в bash нет в принципе: BPF-фильтр системных вызовов для песочницы
  # — программу для ядра умеет собрать только код (libseccomp). В крейте лежит
  # ещё парсер конфигов WG/AWG с тестами на все грабли из docs/GOTCHAS.md §4,
  # но зоны его пока не используют: bash-версия остаётся рабочей до
  # подтверждённого паритета.
  #
  # Бинарь называется vpn-zone-seccomp — со скриптом vpn-zone он не
  # сталкивается, оба спокойно лежат в home.packages.
...
in
{
  ...
  home.packages = [
    ...
    wl-sandbox
    vpn-fs-sandbox
    # Генератор seccomp-фильтра. В PATH он не только ради песочницы: этим же
    # бинарём проверяется, что фильтр вообще работает на твоём ядре
