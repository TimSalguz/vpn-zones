...
#
# ЧЕМ ОТЛИЧАЕТСЯ ОТ vpn-up/vpn-run (machine/nixos/vpn.nix). Те требуют root и
# знают ровно один namespace, прописанный в системной конфигурации. Здесь root не
...
#
# ПОЧЕМУ ЭТО ВООБЩЕ ВОЗМОЖНО БЕЗ ROOT (проверено на этой машине):
#   • непривилегированные user namespace разрешены ядром;
#   • /etc/subuid уже выдаёт тебе диапазон 100000+65536 — из него берётся «root»
...
#   unshare --map-users=0:<subuid>:1 --map-users=1000:1000:1 --setuid 0
...
# Но твой uid 1000 отображён ВТОРЫМ диапазоном, поэтому приложение, вошедшее в
...
# зону из base_config.conf, пока активен системный туннель с тем же конфигом
# (`awg-up`), и не используй один файл для двух зон. Нужны параллельные зоны —
...
let
  ...
  zoneInit = pkgs.writeShellScript "vpn-zone-init" ''
    ...
    address=$(field Address | cut -d, -f1)
    mtu=$(field MTU)
    ...
    case "$epaddr" in
      *[a-zA-Z]*)
        epaddr=$(${pkgs.getent}/bin/getent ahostsv4 "$epaddr" \
    ...
    # --- ТУННЕЛЬ ---
    ${iproute} link add awg0 type amneziawg
    ${awg} setconf awg0 "$stripped"
    [ -n "''${address:-}" ] && ${iproute} -4 addr add "$address" dev awg0
    ...
    # сущность, монтируется при запуске программы (см. vpn-profile-run.py),
    # поэтому один и тот же профиль можно поднять хоть в другой зоне, хоть без
    # VPN вовсе.
    touch "$dir/ready"
    ...
    (
      while :; do
        ${awg} show awg0 > "$dir/status.tmp" 2>/dev/null \
    ...
    if ${awg} show awg0 latest-handshakes 2>/dev/null | ${pkgs.gawk}/bin/awk '{exit ($2>0)?0:1}'; then
    ...
    case "$cmd" in
      ...
      run)
        ...
        if [ -f "$reg" ]; then
          ...
          # живыми. Строку добавляем printf'ом — многострочная переменная с
          # закрывающей кавычкой в нулевой колонке сбила бы снятие общего
        ...
        # см. vpn-profile-run.py.
        exec nsenter --preserve-credentials --keep-caps -U -n -m -t "$p" -- \
          unshare --mount --propagation private -- \
          ${pkgs.python3}/bin/python3 ${./vpn-profile-run.py} \
  ...
  # На python, а не на sed: .desktop — это ini с локализованными ключами и
  # экранированием, и разбирать его построчно значит однажды получить ярлык с
  # поехавшим Exec.
  ...
  vpn-zone-sync = pkgs.writeShellScriptBin "vpn-zone-sync" ''
    exec ${pkgs.python3}/bin/python3 ${./vpn-zone-sync.py} \
